...
ipcam = input("Si usas una cámara ip introduce la IP aquí: ")
...
while True:
    ...
    frame2 = frame.copy()
    mouse = np.zeros((450, 800),dtype=np.uint8)
    ...
    for c in cnts:
        ...
        if area > 1000:
            ...
            cv2.circle(frame2,(x2,y2),grosor,(0,0,0),3)
            cv2.circle(mouse,(x2,y2),grosor,(255,255,255),3)
            #####mover mouse a las coordenadas dadass########
            pywinauto.mouse.move(coords=(x2, y2))
            x1 = x2
            y1 = y2
        else:
            x1, y1 = None, None
    ...
    cv2.imshow("copia", frame2)
    cv2.imshow("moouse", mouse)
